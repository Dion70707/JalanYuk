...
export default function PemesananScreen({ route }) {
  ...
  const handlePemesanan = async () => {
    ...
    try {
      ...
      if (result?.result === 200) {
        Alert.alert(
          "✅ Sukses",
          result.message || "Pemesanan berhasil dibuat."
        );
  ...
  const simpanStrukKeGaleri = async () => {
    try {
      ...
      if (!permission.granted) {
        Alert.alert(
          "Izin Ditolak",
          "Aplikasi tidak memiliki izin menyimpan ke galeri."
        );
        return;
      }
      ...
      Alert.alert(
        "✅ Berhasil",
        "Bukti pemesanan berhasil disimpan ke galeri!"
      );
    } catch (error) {
  ...
  const handleSelesaikan = async () => {
    ...
    if (!transaksiData?.id || transaksiData.id === 0) {
      ...
      Alert.alert(
        "Error",
        "Data transaksi tidak valid. Tidak dapat menyelesaikan pemesanan."
      );
      return;
    }
    ...
    try {
      ...
      if (err.response?.data) {
        Alert.alert(
          "❌ Error",
          err.response.data.message || "Terjadi kesalahan dari server."
        );
      } else {
        Alert.alert("❌ Error", "Tidak dapat menghubungi server.");
      }
