...
export default function FavoritComponent({ navigation }) {
  ...
  const loadFavorites = async () => {
    try {
      ...
      const res = await fetch(`http://172.20.10.3:8080/TrsFavorit/aktif/${userId}`);
      const data = await res.json();

      const ids = data.map(item => item.idWisata);
      setFavoriteIds(ids);
    } catch (err) {
...
const styles = StyleSheet.create({
  ...
  image: { width: '100%', height: 180, backgroundColor: '#ddd', borderRadius: 10 },
  cardContent: {
    flexDirection: 'row',
